...
def build_joint_trajectory_msg(
    points_7dof: np.ndarray,
    joint_names: list[str],
    frame_id: str = "panda_link0",
    dt_per_point: float = 0.05,
) -> JointTrajectory:
    """Build JointTrajectory from (N, 7) array of [x, y, z, roll, pitch, yaw, gripper]."""
    msg = JointTrajectory()
    msg.header.frame_id = frame_id
    msg.joint_names = joint_names
    for i, pt in enumerate(points_7dof):
        jpt = JointTrajectoryPoint()
        jpt.positions = np.asarray(pt, dtype=float).tolist()
        t = i * dt_per_point
        jpt.time_from_start = Duration(sec=int(t), nanosec=int((t % 1) * 1e9))
        msg.points.append(jpt)
    return msg
...
def main():
    ...
    for step in reply:
        ...
        if 'target_object' in step and 'action_id' in step:
            ...
            JOINT_NAMES = ["x", "y", "z", "roll", "pitch", "yaw", "gripper"]
            DT_PER_POINT = 0.05
            ...
            if action_id in (0, 1):
                ...
                if segmap is not None:
                    ...
                    print(f"  Center (y, x): ({center_y_int}, {center_x_int})")
                    center_z_m = depth_image[center_x_int, center_y_int]
                    ...
                    # Build full 7-DoF trajectory (100 points) and publish once for Franka to execute
                    gripper_val = float(robot_current_pose[-1])
                    traj_7dof = np.vstack([np.append(traj[:, i], gripper_val) for i in range(traj.shape[1])])
                    msg = build_joint_trajectory_msg(
                        traj_7dof,
                        joint_names=JOINT_NAMES,
                        frame_id="panda_link0",
                        dt_per_point=DT_PER_POINT,
                    )
                    msg.header.stamp = camera_node.get_clock().now().to_msg()
                    camera_node.traj_pub.publish(msg)
                    print(f"  Published {traj_7dof.shape[0]} waypoints to Franka")

            elif action_id == 2:  # grasp: single waypoint, close gripper
                pose_7dof = np.array(robot_current_pose, dtype=float)
                pose_7dof[6] = 0.0
                msg = build_joint_trajectory_msg(
                    pose_7dof.reshape(1, 7),
                    joint_names=JOINT_NAMES,
                    frame_id="panda_link0",
                    dt_per_point=DT_PER_POINT,
                )
                msg.header.stamp = camera_node.get_clock().now().to_msg()
                camera_node.traj_pub.publish(msg)
                print("  Published grasp (close gripper) waypoint to Franka")

            elif action_id == 3:  # drop: single waypoint, open gripper
                pose_7dof = np.array(robot_current_pose, dtype=float)
                pose_7dof[6] = 0.08
                msg = build_joint_trajectory_msg(
                    pose_7dof.reshape(1, 7),
                ...
                msg.header.stamp = camera_node.get_clock().now().to_msg()
                camera_node.traj_pub.publish(msg)
                print("  Published drop (open gripper) waypoint to Franka")
                #===============================================================
